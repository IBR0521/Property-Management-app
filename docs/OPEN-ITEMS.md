...
| 0b | **Turn on point-in-time recovery if the plan allows it** | There was no backup to restore from. This database has twice been the only copy of something. **Same item as B1 below — counted there, not twice.** |
...
| ~~P4~~ | ~~**The manifests carry no company name**~~ | **Fixed.** A manifest is fetched without credentials, so the company is in the URL instead: `/m/<slug>/app.webmanifest`. The installed icon carries the company's own name, shortened to fit under a home-screen icon. An unknown slug falls back to the generic wording rather than 404ing — an install is not the moment to argue about a URL. |
...
| ~~A2~~ | ~~**`4000 Rent income` is now unposted**~~ | **Done.** Retired (`active = 0`) by migration 050 rather than deleted, so the companies whose older journals used it keep that history. Retiring was not safe before: every report in `reports/financial.js` filtered on `active = 1`, so retiring an account with postings took them off the profit and loss and put the balance sheet out by the same amount. That filter now keeps any account carrying history. |
| ~~A3~~ | ~~**`5000 Repairs` now means only the manager's own costs**~~ | **Already correct, and now held.** `recordInvoice` branches on `ownerBorne`: an invoice against a property reduces owner funds, one with no property is the manager's own and hits 5000. Nothing tested it — `ownerBorne` appeared in no test file — and a posting rule without a test is how two bugs survived ten phases each. Six tests now cover both directions, the tax, the owner's ledger line and parity. |
...
| ~~D~~ | ~~**Recurring charges beyond rent**~~ | **Built in Phase 11.** `recurring_charge` on a lease: pet rent, parking, storage, utilities, amenity. Billed with the rent by the scheduler, prorated on the company's basis, itemised for the tenant, aged from the due date, exported, and brought in from the thirty column names the importer has always warned it could not read — listed on the preview and created only if you tick the box. Manager-payee charges work too, since fees now settle. |
| ~~E~~ | ~~**A background data export for a very large portfolio**~~ | **Not needed.** The premise was that the export holds every table in memory. Measured at 2,000 units it was **1,172MB of heap for a 34MB archive** — worse than the item assumed, and already past a serverless limit. It now reads each table through a cursor and deflates as it goes, keeping only compressed bytes: **40MB**, same archive, validated by Info-ZIP and Python's zipfile. The snapshot that makes the archive one moment is unaffected, because the cursor runs inside the same transaction. |
