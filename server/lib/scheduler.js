...
export function startScheduler() {
  const result = tick("boot");
  console.log(`  scheduler: ${summarise(result)}\n`);
  setInterval(() => tick("interval"), EVERY_MS).unref();
}

export function tick(reason = "manual") {
  const out = {
    reason,
  ...
  for (const company of all("SELECT * FROM company")) {
    Object.assign(out, sumInto(out, generateObligations(company)));
    Object.assign(out, sumInto(out, ageObligations(company)));
    Object.assign(out, sumInto(out, queueObligationReminders(company)));
    Object.assign(out, sumInto(out, openDelinquencies(company)));
    Object.assign(out, sumInto(out, advanceDelinquencies(company)));
    Object.assign(out, sumInto(out, judgePromises(company)));
  }

  out.delivered = drainOutbox();
  out.sessionsPruned = pruneSessions();
  return out;
}
...
/* ==========================================================================
   ...
   ========================================================================== */
function generateObligations(company) {
  const out = { obligationsCreated: 0 };
  const rules = all("SELECT * FROM compliance_rule WHERE company_id = ? AND active = 1", company.id);
...
function ageObligations(company) {
  const r = run(
...
function queueObligationReminders(company) {
  ...
  const staff = all("SELECT email, name FROM staff WHERE company_id = ? AND active = 1", company.id);
  ...
  for (const o of rows) {
    ...
    for (const lead of leads) {
      ...
      if (get("SELECT id FROM outbox WHERE about_type = 'obligation_lead' AND about_id = ?", key)) continue;
...
/* ==========================================================================
   ...
   ========================================================================== */
function openDelinquencies(company) {
  ...
  for (const lease of leases) {
    ...
    const existing = get("SELECT * FROM delinquency WHERE lease_id = ? AND period = ?", lease.id, period);
    if (existing) {
      if (existing.status === "open" && existing.amount_cents !== owed) {
        run("UPDATE delinquency SET amount_cents = ? WHERE id = ?", owed, existing.id);
      }
      continue;
    }
...
function advanceDelinquencies(company) {
  ...
  for (const d of open) {
    ...
    if (target.requires_attorney) {
      run("UPDATE delinquency SET stage = ?, status = 'attorney' WHERE id = ?", target.stage, d.id);
    ...
    run("UPDATE delinquency SET stage = ? WHERE id = ?", target.stage, d.id);
    out.delinquenciesAdvanced++;
    ...
    if (!tpl || !tpl.approved_at) {
      insert("outbox", {
        ...
        to_contact: (get("SELECT email FROM staff WHERE company_id = ? AND active = 1", company.id) || {}).email || "",
    ...
    out.noticesQueued += queueNotice({ company, delinquency: d, step: target, template: tpl, lateDays });
  }
  return out;
}

export function queueNotice({ company, delinquency, step, template, lateDays, sentBy = "system" }) {
  const tenants = all(
...
function judgePromises(company) {
  ...
  for (const p of due) {
    ...
    run("UPDATE payment_promise SET kept = ? WHERE id = ?", kept, p.id);
    // A broken promise puts the case back on the ladder.
    if (!kept) run("UPDATE delinquency SET status = 'open' WHERE id = ? AND status = 'promised'", p.delinquency_id);
    out.promisesJudged++;
  }
  return out;
}
...
function drainOutbox() {
  ...
  const queued = all("SELECT * FROM outbox WHERE status = 'queued' ORDER BY queued_at LIMIT 50");
  let n = 0;
  for (const m of queued) {
    if (DELIVERY.mode === "log") {
      console.log(`[outbox:${m.channel}] -> ${m.to_contact} :: ${m.subject || ""}`);
      run("UPDATE outbox SET status = 'sent', sent_at = ?, attempts = attempts + 1 WHERE id = ?", stamp(), m.id);
      n++;
    }
  }
  return n;
...
export function outboxPending(companyId) {
  return get("SELECT COUNT(*) AS n FROM outbox WHERE company_id = ? AND status = 'queued'", companyId).n;
}
