...
export function registerPortfolio(router) {
  ...
  router.post("/app/portfolio/moveout", (ctx) => {
    const cid = ctx.staff.company_id;
    const lease = one("SELECT * FROM lease WHERE id = ? AND company_id = ?", String(ctx.fields.lease_id || ""), cid);
    const date = String(ctx.fields.moveout_date || "");
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new BadRequest("Give the date the keys came back.");

    tx(() => {
      update("lease", lease.id, { moveout_date: date, status: "ended" });
      update("unit", lease.unit_id, { status: "vacant" });
      insert("audit_log", {
  ...
  /* ======================================================================
     ...
     ====================================================================== */
  router.get("/app/portfolio/u/:id", (ctx) => {
    const cid = ctx.staff.company_id;
    const u = get(
    ...
    const turn = get("SELECT * FROM turn WHERE unit_id = ? AND status = 'open' LIMIT 1", u.id);
    const recent = all(
